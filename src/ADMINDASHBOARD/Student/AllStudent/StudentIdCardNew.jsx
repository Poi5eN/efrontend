...
const StudentIdCardNew = () => {
  ...
  return (
    <div className="flex justify-center">
      <div
        ref={targetRef}
        className="border-2 border-blue-400 w-72 relative rounded-lg p-2"
      >
        <div>
          ...
          <div className="text-center bg-blue-300 pt-[2px] pb-2 mt-2">
            <p className="text-gray-700 font-bold text-sm">
              {studentData.address} <br />{" "}
              <span>Tel: +91{studentData.contact} </span>
            </p>
          </div>
        </div>
        <div className="flex ">
          <div className="w-[50%] h-32 border-4 border-blue-300 mt-2 rounded-md ms-2 overflow-hidden">
        ...
        <div className="mt-2">
          ...
          <p className=" font-bold absolute bottom-2 right-2">Principal</p>
        </div>
      </div>
      <div className=" ms-7 mt-5 ">{!generatingPDF && downloadButton}</div>
    </div>
  );
};
